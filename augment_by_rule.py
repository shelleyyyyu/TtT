import argparse
import jieba
import data_augmentation_rule.delete as delete
import data_augmentation_rule.insert as insert
import data_augmentation_rule.substitution as substitution
import data_augmentation_rule.pronouce as pronouce
import data_augmentation_rule.local_paraphrase as local_paraphrase
import random
...
class DataAugmentationByRule(object):
    def __init__(self, weight, bert_vocab):
        # data_augmentation_rule
        self.comma_dict = get_comma_dict('./data_augmentation_rule/dict/comma_words.txt')
        self.pron_dict = get_pron_dict('./data_augmentation_rule/dict/prononciation.txt')
        self.vocab_dict = get_vocab_dict('./data_augmentation_rule/dict/vocab.txt')
        self.char_shape_dict = get_shape_dict('./data_augmentation_rule/dict/Bakeoff2013_CharacterSet_SimilarShape.txt')
        self.delete_chars = get_delete_chars('./data_augmentation_rule/dict/delete_char_list.txt')
        self.delete_words = get_delete_chars('./data_augmentation_rule/dict/delete_word_list.txt')
    ...
    def augment(self, sentences):
        ...
        for sent_idx, sent in enumerate(sentences):
            ...
            if augment_sent:
                augment_sents.append(''.join(augment_sent))
            else:
